...
* 7일차 (D1~D6 복습 및 객체지향 Todo 프로그램 심화)
---
...
* D1~D6 학습 내용 복습
* 클래스(Class), 객체(Object), 생성자(Constructor), `this` 개념 설명 연습
* 객체 생성 시 메모리(Stack, Heap) 구조 학습
* `List<Todo>`와 배열(Array)의 차이 복습
* 향상된 for문의 동작 원리 복습
* 객체지향 방식으로 Todo 프로그램 직접 구현
* 완료 상태 변경(`exchange()`) 메서드 구현
* `boolean` 타입과 `!`(논리 부정) 연산자 학습
* `List.get()`의 반환값과 객체 메서드 호출 방식 학습

## 배운 점

### 객체 생성과 메모리 구조

객체는 `new`를 사용하여 생성되며 Heap 메모리에 저장된다.

참조 변수는 Stack 메모리에 생성되며 객체의 주소(참조값)를 저장한다.
...
`p`에는 객체 자체가 아니라 Heap에 생성된 `Person` 객체의 참조값이 저장된다.

---

### List.get()의 반환값

`List<Todo>`에서 `get(index)`를 호출하면 해당 위치의 `Todo` 객체를 반환한다.

반환된 객체는 일반 객체와 동일하게 메서드를 호출할 수 있다.

예시

```java
todos.get(num - 1).exchange();
```
...
`boolean`은 `true`와 `false` 두 가지 값만 가진다.

`!` 연산자를 사용하면 현재 값을 반대로 변경할 수 있다.
...
기존의 `if-else`문보다 간결하게 상태를 변경할 수 있다.

---

### 변수와 문자열의 차이

변수와 문자열은 서로 다른 의미를 가진다.
...
는 `"title"`이라는 문자열 자체가 저장된다.

---

### 객체 메서드 호출

`List.get()`으로 반환된 객체는 일반 객체와 동일하게 사용할 수 있다.
...
객체를 반환받으면 해당 객체의 메서드를 호출하여 객체의 상태를 변경할 수 있다.

## 느낀 점

오늘은 새로운 내용을 배우기보다 지금까지 학습한 내용을 다시 설명하고 직접 구현하면서 부족한 부분을 확인하는 시간이었다. 특히 `List.get()`이 객체를 반환한다는 개념과 반환된 객체의 메서드를 호출하는 흐름을 이해하면서 객체지향 코드의 동작 방식을 더 명확하게 이해할 수 있었다. 또한 `done = !done;`처럼 `boolean` 타입의 특성을 활용하여 더 간결하게 코드를 작성할 수 있다는 점도 새롭게 배웠다. 앞으로도 직접 구현하고 부족한 부분을 보완하는 방식으로 꾸준히 복습하며 객체지향 개념을 확실히 익혀나가고 싶다.
